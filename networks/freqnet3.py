...
class FreqNet(nn.Module):
    ...
    def __init__(self, block=Bottleneck, layers=[3, 4], num_classes=1, zero_init_residual=False):
        ...
        # ──────────────── Color Relationship Learning (CRL) parameters ────────────────
        # Pointwise conv: learns cross-channel color features (3 → 16)
        self.crl_weight1 = nn.Parameter(torch.randn(16, 3, 1, 1).cuda())
        self.crl_bias1   = nn.Parameter(torch.randn(16).cuda())

        # Pointwise conv: deeper nonlinear projection (16 → 32)
        self.crl_weight2 = nn.Parameter(torch.randn(32, 16, 1, 1).cuda())
        self.crl_bias2   = nn.Parameter(torch.randn(32).cuda())

        # Final projection back to 3 channels to preserve original model compatibility (32 → 3)
        self.crl_weight3 = nn.Parameter(torch.randn(3, 32, 1, 1).cuda())
        self.crl_bias3   = nn.Parameter(torch.randn(3).cuda())

        # FCL 1: Spectral convs for FFT amplitude and phase
        self.fcl1_amp = conv1x1(3, 3)
        self.fcl1_phase = conv1x1(3, 3)
    ...
    def crl(self, x):
        # Color Relationship Learning: learn color-channel interactions with 1×1 convs
        x = F.relu(F.conv2d(x, self.crl_weight1, self.crl_bias1, stride=1, padding=0))
        x = F.relu(F.conv2d(x, self.crl_weight2, self.crl_bias2, stride=1, padding=0))
        x = F.relu(F.conv2d(x, self.crl_weight3, self.crl_bias3, stride=1, padding=0))
        return x
